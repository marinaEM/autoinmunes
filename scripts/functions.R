...
########     (previously dividing the matrixes) 
##################################################################  
...
#### Function to simulate the effect of a drug by KO of all its gene targets, previously 
#####  load trans_data, pathways and metadata without controls - PLOT of log2 FC
#####  for the new dataset validation cohort
###############################################

simulate_drug_validation <- function(drug, expression_matrix = trans_data, metadata_matrix_controls = metadata_init, 
                          drug_table = drug_targetsDF, pathways_db = pathways, folder = NA){
  if(is.na(folder)){
    folder = ""
  }else{
  ...
  if(any(possible_targets %in% pathways_db$all.genes)){
    ...
    ## Using Hipathia to compute the signal (subpathways) 
    results_KO <- hipathia(exp_data_KO_C, pathways_db, decompose = FALSE, verbose=FALSE)
    saveRDS(results_KO, file = here("rds","KO_drugs", folder, paste0(drug,"_KO_", paste(targets_name, collapse="_") ,"_validation_results.rds")))
    ...
    metadata <- rbind(metadata_KO, metadata_matrix_controls)
    target_matrix_KO <- data.frame(samples = colnames(path_vals_KO), 
                                   cluster = metadata$cluster[match(colnames(path_vals_KO), metadata$samples)],
                                   diagnosis = metadata$diagnosis[match(colnames(path_vals_KO), metadata$samples)],
                                   age = metadata$age[match(colnames(path_vals_KO), metadata$samples)],
                                   gender = metadata$gender[match(colnames(path_vals_KO), metadata$samples)],
                                   center = metadata$center[match(colnames(path_vals_KO), metadata$samples)],
                                   pool = gsub(pattern = "pool", replacement = "", metadata$pool[match(colnames(path_vals_KO), metadata$samples)]),
                                   RIN = metadata$RIN[match(colnames(path_vals_KO), metadata$samples)],
                                   type = metadata$type[match(colnames(path_vals_KO), metadata$samples)],
                                   stringsAsFactors = F)
    ...
    if(dim(top_LimmaALL)[1]>0){
      ...
      write.table(anot_all, file = here("results", "KO_drugs", folder, paste0(drug, "_KO_deregCir_validation_afterLimmaAnnot.tsv")), sep = "\t", quote = F,
                  row.names = F, col.names = T)
